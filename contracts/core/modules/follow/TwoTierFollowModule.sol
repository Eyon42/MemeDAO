...
contract TwoTierFollowModule is IFollowModule, FeeModuleBase, FollowValidatorFollowModuleBase {
    ...
    mapping(uint256 => mapping(uint256 => FollowTier)) _followTier;
        ...
        returns (bytes memory)
    {
        ...
        _dataByProfile[profileId].amount = amount; // Membership amount
    ...
    function processFollow(
        ...
        (bool member, uint256 followerId) = abi.decode(data, (bool, uint256));
        require(IERC721(HUB).ownerOf(followerId) == follower, "You don't own that profile"); // IERC721 in this case is the LensHub contract.
        require(
            _followTier[profileId][followerId] == FollowTier.NonFollower ||
                (_followTier[profileId][followerId] == FollowTier.Follower && member)
        );

        if (member) {
            _followTier[profileId][followerId] = FollowTier.Member;
            ...
            _followTier[profileId][followerId] = FollowTier.Follower;
        }
    }

    function getFollowStatus(uint256 followed, uint256 follower) public view returns (FollowTier) {
        return _followTier[followed][follower];
    }
